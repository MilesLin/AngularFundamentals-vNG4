...
export class EventService {
  ...
  getEvents():Observable<IEvent[]> {
    return this.http.get(`${environment.host}/api/events`).map((response: Response) => {
        return <IEvent[]>response.json();
    }).catch(this.handleError);
  }

  getEvent(id:number):Observable<IEvent> {
    return this.http.get(`${environment.host}/api/events/` + id).map((response: Response) => {
        return <IEvent>response.json();
    }).catch(this.handleError);
  }
  ...
  saveEvent(event): Observable<IEvent> {
    ...
    return this.http.post(`${environment.host}/api/events`, JSON.stringify(event), options).map((response: Response) => {
      return response.json();
    }).catch(this.handleError);
  }

  searchSessions(searchTerm: string) {
    return this.http.get(`${environment.host}/api/sessions/search?search=` + searchTerm).map((response: Response) => {
        return response.json()
    }).catch(this.handleError);
  }
